...
public:
  ...
  GetInformations(const std::filesystem::path &path) const {
    ...
    while (std::getline(file, line)) {
      if (line.empty()) {
        throw std::logic_error("Empty line encountered.");
        break;
      }
      ...
      if (isAction) {
        ...
        if (!isValid)
          throw std::logic_error(std::format("Invalid action line: {}", line));
        ...
        if (!isValid)
          throw std::logic_error(std::format("Invalid result line: {}", line));
...
private:
  const static inline std::filesystem::path Root{
      "D:\\Documents\\cpp_learn\\Orderbook\\OrderbookTest"};
...
INSTANTIATE_TEST_CASE_P(
    ...
    googletest::ValuesIn({"Match_GoodTillCancel.txt", "Match_FillAndKill.txt",
                          ...
                          "Modify_Side.txt", "Match_Market.txt"}));
